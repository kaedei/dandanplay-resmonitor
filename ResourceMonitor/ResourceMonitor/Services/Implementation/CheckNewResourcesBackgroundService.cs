...
namespace ResourceMonitor.Services.Implementation
{
    ...
    public class CheckNewResourcesBackgroundService : BackgroundService
    {
        ...
        private async Task DownloadRule(AutoDownloadRule rule)
        {
            ...
            //获得需要下载的新资源
            var resourceList = await _resApi.List(rule.keyword, rule.subgroupId, rule.typeId);
            ...
            foreach (var resourceInfo in filtered)
            {
	            ...
	            if (exists)
	            {
		            _logger.LogDebug($"任务已经存在，跳过创建 {hash}");
		            continue;
	            }
	            //解析磁力链至种子文件
			...
			int limitFileSize, bool logDetails = false)
		{
			...
			if (chooseNewerIfDuplicate)
			{
				...
				foreach (var resource in filtered)
				{
					...
					var normalizedTitle = Regex.Replace(resource.Title, @"\W|v3|v2", string.Empty, RegexOptions.IgnoreCase);
					if (resource.Title.Contains("v3") || resource.Title.Contains("V3"))
					{
						var replacements = filtered.Where(res =>
							res.Title != resource.Title &&
							normalizedTitle == Regex.Replace(res.Title, @"\W|v2", string.Empty, RegexOptions.IgnoreCase));
			...
			if (limitFileSize > 0)
			{
				...
				if (logDetails)
				{
					foreach (var info in filtered)
					{
						_logger.LogDebug($"Filtered By LimitFileSize {limitFileSize}. title={info.Title} size={info.FileSizeInMB} magnet={info.Magnet}");
					}
				}
			}
			...
			if (logDetails)
			{
				foreach (var info in filtered)
				{
					_logger.LogDebug($"Filtered By PublishDate. title={info.Title} date={info.PublishDateTime} magnet={info.Magnet}");
				}
			}
