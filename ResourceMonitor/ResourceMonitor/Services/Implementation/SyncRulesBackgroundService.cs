...
namespace ResourceMonitor.Services.Implementation
{
    ...
    public class SyncRulesBackgroundService : BackgroundService
    {
        ...
        {
            _logger.LogInformation("SyncRulesBackgroundService 开始运行.");
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                //每十分钟运行一次
                _logger.LogInformation("等待 10 分钟后重新执行");
                await Task.Delay(TimeSpan.FromMinutes(10), stoppingToken);
            }

            _logger.LogInformation("因触发取消，SyncRulesBackgroundService 终止运行.");
        }
        ...
        private async Task<bool> DoWork()
        {
            ...
            _logger.LogInformation($"SyncRulesBackgroundService 在后台第 {count} 次运行");
           ...
           //之前登录过，先尝试直接刷新 jwt token
            ...
            _logger.LogInformation($"SyncRulesBackgroundService 在后台运行完成");
